...
class BcolzArrayIterator(object):
    ...
    def __init__(self, X, y=None, w=None, batch_size=32, shuffle=False, seed=None):
        if y is not None and len(X) != len(y):
            raise ValueError(
                f'X (features) and y (labels) should have the same lengthFound: X.shape = {X.shape}, y.shape = {y.shape}'
            )
        if w is not None and len(X) != len(w):
            raise ValueError(
                f'X (features) and w (weights) should have the same lengthFound: X.shape = {X.shape}, w.shape = {w.shape}'
            )
    ...
    def next(self):
        with self.lock:
            ...
            for _ in range(self.chunks_per_batch):
                ...
                if self.y is not None: batches_y.append(self.y[idx: idx + current_batch_size])
                if self.w is not None: batches_w.append(self.w[idx: idx + current_batch_size])
